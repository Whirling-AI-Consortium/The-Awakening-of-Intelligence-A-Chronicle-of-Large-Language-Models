...
LoRA的基础是一个简单但强大的数学观察。考虑一个标准的全量微调过程。对于一个线性层 $y = Wx$，我们会计算梯度 $\frac{\partial L}{\partial W}$，然后按照优化算法（比如Adam）更新参数 $W$。完全微调后，参数从 $W_0$ 变为 $W_0 + \Delta W$。
...
具体地，假设原始权重矩阵 $W_0$ 的维度是 $d_{out} \times d_{in}$（比如在Transformer中，一个前馈层的权重可能是 $4096 \times 4096$）。完全微调需要优化这个矩阵的所有 $d_{out} \times d_{in}$ 个参数。但根据LoRA的假设，我们只需要优化 $A$ 和 $B$，参数数量变为 $(d_{out} + d_{in}) \times r$。
...
从实现的角度，LoRA的应用相当直接。对于Transformer中的线性层，我们用LoRA模块替代完整的权重更新。论文提出了一个具体的初始化方案：$A$ 从高斯分布初始化，$B$ 初始化为零。这确保了在训练开始时，$\Delta W = BA = 0$，使得初始的模型行为与基础模型相同。
